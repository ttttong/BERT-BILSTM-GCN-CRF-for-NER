...
from bilstm_crf.bilstm_crf_layer_pos import BiLSTM_CRF   # bilstm后只加了pos
from bilstm_crf.bilstm_gcn import BiLSTM_GCN_CRF     # bilstm输入到gcn
from bilstm_crf.bilstm_gcn2 import BiLSTM_GCN_CRF2   # bilstm+gcn
...
class InputExample(object):
    ...
    def __init__(self, guid, text, label=None, pos=None, mark=None):
        """
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, pos_ids, mark_ids, forward, backward, is_real_example=True):
        ...
        with open('./data/{}/{}.txt'.format(FLAGS.dataset, set_type), 'r', encoding='utf-8') as f:
            ...
            for line in f:
                ...
                if len(contents) == 0:
                    ...
                    n = ' '.join([str(mark) for mark in marks])
                    l = ' '.join([label for label in labels if len(label) > 0])
                    w = ' '.join([word for word in words if len(word) > 0])
                    lines.append([w, l, s, n])
                    words = []
                    labels = []
                    poses = []
                    marks = []
                ...
                mark = mark2id[line.strip().split(' ')[-1]]
                words.append(word)
                labels.append(label)
                poses.append(pos)
                marks.append(mark)
...
class NerProcessor(DataProcessor):
    def _create_example(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            mark = tokenization.convert_to_unicode((line[-1]))
            example.append(InputExample(guid=guid, text=text, label=label, pos=pos, mark=mark))
        return example
    ...
    def get_labels(self):
        ...
        """
        return ["X", "B-剧种", "I-剧种", "B-人名", "I-人名", "B-剧目", "I-剧目", "B-乐器", "I-乐器", "B-地点", "I-地点", "B-唱腔曲牌", "I-唱腔曲牌",
                "B-脚色行当", "I-脚色行当", "O", "[CLS]", "[SEP]"]
...
def file_based_convert_examples_to_features(
    ...
    for ex_index, (example, a_fw, a_bw) in enumerate(zip(examples, forward, backward)):
        ...
        features["mark_ids"] = create_int_feature(feature.mark_ids)
...
def file_based_input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    name_to_features = {
        ...
        "mark_ids": tf.FixedLenFeature([seq_length], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, label_ids, pos_ids, mark_ids, num_labels, forward, backward, use_one_hot_embeddings):
    """
    ...
    #     with open('./data/{}/mark2id.pkl'.format(FLAGS.dataset), 'rb') as rf2:
    #         mark2id = pickle.load(rf2)
    #     num_mark = len(mark2id)
    #     mark_embeddings = tf.one_hot(mark_ids, num_mark, axis=-1)  # 维度 pos_ids*num_pos
    #     embeddings = tf.concat([embeddings, pos_embeddings], -1)  # 加长embedding向量
    #     embeddings = tf.concat([embeddings, mark_embeddings], -1)  # 加长embedding向量
    ...
    if FLAGS.bilstm or FLAGS.crf:
        ...
        if FLAGS.gcn == 0:
            ...
            loss, predict = bilstm_crf.add_bilstm_crf_layer(pos_ids, mark_ids)
        elif FLAGS.gcn == 1:
            bilstm_crf2 = BiLSTM_GCN_CRF(embedded_chars=embeddings, lstm_size=FLAGS.lstm_size, cell_type=FLAGS.cell,
                                        num_layers=FLAGS.num_layers,
                                        dropout_rate=FLAGS.dropout_rate, num_labels=num_labels,
                                        max_seq_length=max_seq_length,
                                        labels=label_ids,
                                        lengths=lengths, is_training=is_training, bilstm=FLAGS.bilstm, crf=FLAGS.crf,
                                        forward=forward, backward=backward)
            loss, predict = bilstm_crf2.add_bilstm_crf_layer(pos_ids, mark_ids)
        elif FLAGS.gcn == 2:
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        (total_loss, pred_ids) = create_model(
            ...
            label_ids, pos_ids, mark_ids, num_labels, forward, backward, use_one_hot_embeddings)
