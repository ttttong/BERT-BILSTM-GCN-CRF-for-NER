...
from bilstm_crf.bilstm_crf_layer_cnn1 import BiLSTM_CRF_CNN
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, segment_ids, label_ids, pos_ids, is_real_example=True):
...
class NerProcessor(DataProcessor):
    ...
    def get_labels(self):
        ...
        """
        return ["X", "B-剧种", "I-剧种", "B-剧目", "I-剧目", "B-人名", "I-人名", "B-乐器", "I-乐器", "B-地点", "I-地点", "B-唱腔曲牌", "I-唱腔曲牌", "B-脚色行当", "I-脚色行当", "O", "[CLS]", "[SEP]"]
...
def file_based_convert_examples_to_features(
    ...
    for ex_index, example in enumerate(examples):
        ...
        # def create_list_int_feature(value_list):
        #     f = tf.train.FeatureList(feature=[tf.train.Feature(int64_list=tf.train.Int64List(value=list(values))) for values in value_list])
        #     return f
...
def create_model(bert_config, is_training, input_ids, input_mask,
                 segment_ids, label_ids, pos_ids, num_labels, use_one_hot_embeddings):
    """
    ...
    if FLAGS.bilstm or FLAGS.crf:
        ...
        if FLAGS.cnn:
            bilstm_crf = BiLSTM_CRF_CNN(embedded_chars=embeddings, lstm_size=FLAGS.lstm_size, cell_type=FLAGS.cell,
                                    num_layers=FLAGS.num_layers,
                                    dropout_rate=FLAGS.dropout_rate, num_labels=num_labels, max_seq_length=max_seq_length,
                                    labels=label_ids,
                                    lengths=lengths, is_training=is_training, bilstm=FLAGS.bilstm, crf=FLAGS.crf, cnn=FLAGS.cnn)
            loss, predict = bilstm_crf.add_bilstm_crf_layer()
        else:
            bilstm_crf = BiLSTM_CRF(embedded_chars=embeddings, lstm_size=FLAGS.lstm_size, cell_type=FLAGS.cell,
                                    num_layers=FLAGS.num_layers,
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        (total_loss, pred_ids) = create_model(
            ...
            label_ids, pos_ids, num_labels, use_one_hot_embeddings)
