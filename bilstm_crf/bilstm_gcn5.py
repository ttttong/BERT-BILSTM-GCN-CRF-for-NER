...
class BiLSTM_GCN_CRF3:
    ...
    def bilstm_layer(self, hidden):
        with tf.variable_scope('bilstm_layer'):
            ...
            output2, _2 = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw,
                                                        hidden, dtype=tf.float32)
            output = tf.concat(values=[output, output2], axis=2)
        return output
