...
class BiLSTM_GCN_CRF:
    ...
    def GCN_layer(self, hidden, A_fw, A_bw):
        ...
        W0_fw = tf.Variable(tf.random_uniform([self.lstm_size * 2, self.lstm_size], 0, 0.1), name='W0_fw')
        ...
        # bw
        W0_bw = tf.Variable(tf.random_uniform([self.lstm_size * 2, self.lstm_size], 0, 0.1), name='W0_bw')
    ...
    def project_bilstm_layer(self, gcn_output, pos_ids):
        ...
        with tf.variable_scope('project_bilstm_layer'):
            ...
            #     pos2id = {}
                ...
                # mark_embeddings = tf.reshape(mark_embeddings, shape=[-1, num_mark])
                # hidden = tf.concat([hidden, pos_embeddings], -1)  # 加长embedding向量
                # hidden = tf.concat([hidden, mark_embeddings], -1)  # 加长embedding向量
