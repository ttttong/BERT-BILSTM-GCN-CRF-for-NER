...
class BiLSTM_CRF_CNN:
    ...
    def cnn_layer(self, lstm_output):
        with tf.variable_scope('cnn_layer'):
            ...
            filter1 = tf.get_variable(
                "filter",
                shape=[3, self.embedding_size, 30],
                dtype=np.float32,
                initializer=self.initializer)
            output2 = tf.nn.conv1d(self.embedded_chars, filters=filter1, padding="SAME", stride=1)
    ...
    def project_bilstm_layer(self, cnn_output):
        """
        ...
        with tf.variable_scope('project_bilstm_layer'):
            ...
            output = tf.reshape(cnn_output, shape=[-1, self.lstm_size * 2+self.kernel_size])
    ...
    def add_bilstm_crf_layer(self):
        ...
        if not self.bilstm:
            ...
            # project layer
            logits = self.project_bilstm_layer(cnn_output)
